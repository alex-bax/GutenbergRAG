...
*context retrieval/search, reranking, answer generation*
...
💡 It's clear that `search` and `rerank_total`, that the LLM re-ranker is very slow with ~20 secs. It'll worth to investigate faster reranking approaches
...
The 422 error is made on purpose to test the HTTP 4XX visualization. The call is made with: `/v1/books/search?title=NO`. The `title` parameter is less than 2 characters, thus failing the minimum length requirement and triggering the 422 error.
